...
AS $function$
      select
        case when (select mode from projects where id = $2) = 'sub' then
          exists(select true from common_schema.subscriptions s where status in('active', 'canceled') and s.project_id = (select common_id from projects where id = $2 limit 1) and s.user_id = current_user_uuid())
        else
...
AS $function$
      select
        case when (select mode from projects where id = $2) = 'sub' then
          exists(select true from common_schema.subscriptions s where s.status in('active', 'canceled') and s.reward_id = (select common_id from rewards where id = $1 limit 1) and s.user_id = current_user_uuid())
        else
